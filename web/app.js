    ...
    function initListeners() {
        ...
        const toggleMap = [
            ...
            { el: DOM.showTruncMarkers, prop: 'showTruncMarkers' }
                >>>>>>> 44fd91d (web: fix tab icon clipping, badge spacing, and settings responsiveness)
        ];
        ...
        const SEPARATORS = {
            vertical: { type: 'pixel', glyph: '▎' },
            bar: { type: 'glyph', glyph: '│' },
            thick_bar: { type: 'glyph', glyph: '┃' },
            thin: { type: 'pixel', glyph: '▏', fixedWidth: 2 },
            thick: { type: 'pixel', glyph: '▌', fixedWidth: 5 },
            slant: { type: 'glyph', glyph: '' },
            slope: { type: 'glyph', glyph: '' },
            padded_slant: { type: 'glyph', glyph: ' ' },
            none: { type: 'glyph', glyph: ' ' }
        };
        ...
        function renderTabline() {
            ...
            if (state.showTruncMarkers && leftCount > 0) {
                ...
                leftMarker.className = 'tab-overflow-marker';
                leftMarker.title = 'Previous hidden buffers (click or S-TAB)';
                leftMarker.innerHTML = `<span></span><span class="overflow-count">${leftCount}</span>`;
                leftMarker.addEventListener('click', () => cycleBuffer(-1));
                DOM.bufferlineBar.appendChild(leftMarker);
            }
            ...
            visible.forEach((buf) => {
                ...
                tabEl.style.paddingTop = `${state.tabPaddingVertical}px`;
                tabEl.style.paddingBottom = `${state.tabPaddingVertical}px`;

                // separator / indicator
                const indEl = document.createElement('div');
                indEl.className = 'tab-indicator';

                const styleDef = SEPARATORS[state.separatorStyle] || SEPARATORS.vertical;

                if (styleDef.type === 'pixel') {
                    const w = styleDef.fixedWidth || state.indicatorWidth;
                    indEl.style.width = `${w}px`;
                } else {
                    indEl.textContent = styleDef.glyph;
                    indEl.style.color = isActive ? 'var(--emacs-indicator)' : 'transparent';
                }
                tabEl.appendChild(indEl);

                // pre-pad
                const prePad = document.createElement('span');
                prePad.className = 'tab-pre-pad';
                tabEl.appendChild(prePad);
                ...
                if (state.showIcons && buf.icon) {
                    ...
                    icon.textContent = buf.icon;
                    icon.style.color = buf.iconColor || 'var(--emacs-fg)';
                    if (state.iconSpacing > 0) {
                        icon.style.marginRight = `${state.iconSpacing * 8}px`;
                    }
                ...
                if (state.showDiagnostics && buf.diags) {
                    const diag = document.createElement('span');
                    diag.className = `tab-diag-badge ${buf.diags.err > 0 ? 'error' : 'warning'}`;
                    diag.textContent = buf.diags.err > 0 ? `  ${buf.diags.err}` : `  ${buf.diags.warn}`;
                    tabEl.appendChild(diag);
                }

                // horizontal right padding
                const postPad = document.createElement('span');
                postPad.style.display = 'inline-block';
                postPad.style.width = `${state.tabPaddingHorizontal * 8}px`;
                tabEl.appendChild(postPad);
            ...
            if (state.showTruncMarkers && rightCount > 0) {
                ...
                rightMarker.className = 'tab-overflow-marker';
                rightMarker.title = 'Next hidden buffers (click or TAB)';
                rightMarker.innerHTML = `<span class="overflow-count">${rightCount}</span><span style="margin-left:4px"></span>`;
                rightMarker.addEventListener('click', () => cycleBuffer(1));
                DOM.bufferlineBar.appendChild(rightMarker);
            }
        }
